...
Page({
  ...
  onLoad: function (options) {
    ...
    userApi.requestAppApi_POST(app.globalData.mainDomain + "XcxApi/getChafenToken", {
    }, function (res) {
      that.initdata(res.data.data.openid,res.data.data.token);
      that.chafenopenid = res.data.data.openid;
      that.chafentoken = res.data.data.token;
    });

  },
  initdata: function (chafenopenid,chafentoken){
    const that = this;
    userApi.requestAppApi_POST(app.globalData.mainDomain + "ApiJiucuo/xcx_chafen_template", {
      openid: chafenopenid, token:chafentoken, channel: that.data.channel
    }, function (res) {
      ...
      for (var i = 0; i < fileds.length; i++) {
        if (fileds[i].type == 1) {
          ...
          that.setData({
            imgurl: app.globalData.mainQueryDomain + "ApiJiucuo/xcx_chafen_chaptcha?openid=" + chafenopenid + "&token=" + chafentoken + "&tt=" + Math.random() + "&channel=" + that.data.channel,
          });
        }
      }
      that.setData({
  ...
  refresh:function(){
    ...
    // });
    that.setData({
      imgurl: app.globalData.mainQueryDomain + "ApiJiucuo/xcx_chafen_chaptcha?openid=" + that.chafenopenid + "&token=" + that.chafentoken + "&tt=" + Math.random() + "&channel=" + that.data.channel,
    });  
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
